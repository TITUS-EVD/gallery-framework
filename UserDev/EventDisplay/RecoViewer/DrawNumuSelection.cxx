...
Track2D DrawNumuSelection::getTrack2D(recob::Track track, unsigned int plane) {
  // Track2D result;
  auto geoHelper = larutil::GeometryHelper::GetME();
  std::vector<std::pair<float, float> > _track;
  _track.reserve(track.NumberTrajectoryPoints());
  for (unsigned int i = 0; i < track.NumberTrajectoryPoints(); i++) {
    ...
    try {
      ...
      _track.push_back(std::make_pair(point.w, point.t));
    } catch (...) {
      continue;
    }
  }

  return Track2D(_track);
}

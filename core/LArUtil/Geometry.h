...
public:
    ...
    galleryfmwk::geo::SigType_t SignalType(const UInt_t channel)  const;

    /// return the signal type for a given plane
    galleryfmwk::geo::SigType_t PlaneToSignalType(const UChar_t plane)  const;

    /// return the view type for a given channel
    galleryfmwk::geo::View_t View(const UInt_t channel)  const;

    /// return the view type for a given plane
    galleryfmwk::geo::View_t PlaneToView(const UChar_t plane) const;

    /// return vector of possible views in the detector
    std::set<galleryfmwk::geo::View_t>  const Views() const;
...
public:
    ...
    Double_t   WirePitch(const galleryfmwk::geo::View_t view) const;

    /// assumes all wires in the view have the same angle
    Double_t   WireAngleToVertical(galleryfmwk::geo::View_t view) const;
...
private:
    ...
    std::vector<galleryfmwk::geo::View_t>    fViewType;

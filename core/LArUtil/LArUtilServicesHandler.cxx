...
namespace larutil {
  ...
  std::unique_ptr<geo::WireReadoutGeom> LArUtilServicesHandler::GetWireReadout(std::string fcl_file_name)
  {
    ...
    // geometry setup (it's special)
    const std::unique_ptr<geo::GeometryCore> _geom = GetGeometry(fcl_file_name);
    const geo::GeometryCore* TempGeom = new geo::GeometryCore(*_geom);
    ...
    delete TempGeom; //may break the wireReadout object
  ...
  std::unique_ptr<detinfo::DetectorPropertiesStandard> LArUtilServicesHandler::GetDetProperties(std::string fcl_file_name)
  {
    std::string configFile = fcl_file_name;
    fhicl::ParameterSet config;
    cet::filepath_lookup_after1 policy("FHICL_FILE_PATH");
